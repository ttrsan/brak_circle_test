...
変更内容:
- 加入方式 / メンバー / サークルID の表示領域を削除
- スマホ表示時のサークル情報を簡略化
- スマホ表示時に入力欄がチャット欄の上へ出るようHTML/CSS両方で修正


変更内容:
- テストサークル下へサイト説明文を統一配置
- 「禁止ワードの確認ができます。」を追加


変更内容:
- PC / スマホ両方で「挨拶」ブロックを削除
- サイト説明はテストサークル下のみ表示


変更内容:
- 不要な greeting-card ブロックを削除


変更内容:
- スマホ表示時の mobile-greeting の高さ制限を解除
- 3行すべて表示されるよう修正
